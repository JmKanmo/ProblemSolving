...
bool checkBottomHorse(int number, int prevIdx1, int prevIdx2)
{
	if (board[prevIdx1][prevIdx2].horses.front().number == number) {
		return true;
	}
	else {
		return false;
	...
	while (++turn) {
		if (turn > 1000) {
			...
			for (int i = 1; i <= K; i++) {
				...
				if (checkBottomHorse(i, horse[i].idx1, horse[i].idx2) == false) 
					continue;
